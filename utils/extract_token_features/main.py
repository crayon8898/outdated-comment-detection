...
model = Word2Vec.load("/Users/chenyn/chenyn's/研究生/DataSet/My/词向量/from_changes_word_vector.model")
...
def get_all_token_change_sim(filepath):
    try:
        oldComment, oldCode, newComment, newCode = getCommentAndCode(filepath)
        oldComment, oldCode, newComment, newCode = useCupToGetTxt(oldComment), useCupToGetTxt(oldCode), useCupToGetTxt(
            newComment), useCupToGetTxt(newCode)
        old2old_sim = {word: sim_word2sentence(word, oldCode.split()) for word in oldComment.split()}
        old2new_sim = {word: sim_word2sentence(word, newCode.split()) for word in newComment.split()}
        old2old_sim_t = [sim_word2sentence(word, oldCode.split()) for word in oldComment.split()]
        old2new_sim_t = [sim_word2sentence(word, newCode.split()) for word in newComment.split()]
        change_sim = {oldComment.split()[i]: abs(v - old2new_sim_t[i]) for i, v in enumerate(old2old_sim_t)}
        all = sum(change_sim.values())
        return all
    except:
        return 0.0
...
def get_sims(filepath):
    ...
    if getCommentAndCode(filepath) != None:
        oldComment, oldCode, newComment, newCode = getCommentAndCode(filepath)
    else:
        return 0.0, 0.0, 0.0
    oldComment = useCupToGetTxt(oldComment)
    oldCode = useCupToGetTxt(oldCode)
    newCode = useCupToGetTxt(newCode)

    oldComment = [word for word in oldComment.split()]

    oldCode = [word for word in oldCode.split()]
    changed_sen = [word for word in changed_sen.split()]
    newCode = [word for word in newCode.split()]
    # print(oldComment)
    # print(oldCode)
    # print(newCode)
    # print(changed_sen)
    return round(sim_sen2sen(oldComment, oldCode), 3), round(sim_sen2sen(oldComment, newCode), 3), round(
        sim_sen2sen(oldComment, changed_sen), 3)
...
def getFeatures(filepath):
    with open(filepath) as f:
        ...
        commentByCCSet = float(format(lineNumOfOldComment / (lineNumOfOldCode + lineNumOfOldComment), '.2f'))
        lineNumOfChanged = len(setOfChangedLine)
        if lineNumOfOldCode != 0:
            changedLineByAllCodeLine = float(format((lineNumOfChanged / lineNumOfOldCode), '.2f'))
        else:
            changedLineByAllCodeLine = 0.00
        ...
        for key in dictOfOldCommentPos.keys():
            if len(getTokens(oldComment)) != 0:
                dictOfOldCommentPos[key] = format(dictOfOldCommentPos[key] / len(getTokens(oldComment)), '.2f')
            else:
                dictOfOldCommentPos[key] = 0.00
        for key in dictOfOldCodePos.keys():
            if len(getTokens(oldCode)) != 0:
                dictOfOldCodePos[key] = format(dictOfOldCodePos[key] / len(getTokens(oldCode)), '.4f')
            else:
                dictOfOldCodePos[key] = 0.00
        for key in dictOfNewCodePos.keys():
            if len(getTokens(newCode)) != 0:
                dictOfNewCodePos[key] = format(dictOfNewCodePos[key] / len(getTokens(newCode)), '.4f')
            else:
                dictOfNewCodePos[key] = 0.00
        for key1 in dictOfNewCodePos.keys():
            codePosChange[key1] = format(abs(float(dictOfNewCodePos[key1]) - float(dictOfOldCodePos[key1])) * 10, '.2f')

        oldCommentPos = dictOfOldCommentPos

        newfile = open(("/Users/chenyn/chenyn's/研究生/DataSet/My/features_10_16/") + os.path.split(filepath)[1], 'w')
        ...
        newfile.write('lineNumOfNewCode:' + str(lineNumOfNewCode) + '\n')
        ...
        newfile.write('commentByCCSet:' + str(commentByCCSet) + '\n')
        ...
        newfile.write('cmt2cd_sim_before:' + str(cmt2cd_sim_before) + '\n')
        newfile.write('cmt2cd_sim_after:' + str(cmt2cd_sim_after) + '\n')
        newfile.write('cmt2ch_sim:' + str(cmt2ch_sim) + '\n')
        newfile.write('sim_change:' + str(round(sim_change, 3)) + '\n')
        newfile.write('all_token_change_sim:' + str(round(all_token_change_sim, 5)) + '\n')
        newfile.close()
...
if __name__ == '__main__':
    oldComment, oldCode, newComment, newCode = getCommentAndCode('/Users/chenyn/Desktop/0169InlineTagHandler.java')

    oldComment, oldCode, newComment, newCode=useCupToGetTxt(oldComment),useCupToGetTxt(oldCode),useCupToGetTxt(newComment),useCupToGetTxt(newCode)



    oldComment = [ch for ch in str(oldComment).split(' ') if not str(ch) == '']
    oldCode = [ch for ch in str(oldCode).split(' ') if not str(ch) == '']
    newComment = [ch for ch in str(newComment).split(' ') if not str(ch) == '']
    newCode = [ch for ch in str(newCode).split(' ') if not str(ch) == '']
    #
    print(oldComment)
    print(oldCode)
    print(newComment)
    ...
    #
    for x in oldComment:
        s1=sim_word2sentence(x,oldCode)
        s2=sim_word2sentence(x,newCode)
        print(s1)
        print(s2)
